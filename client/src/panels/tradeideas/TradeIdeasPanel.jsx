...
export default function TradeIdeasPanel({ onSelect }) {
  const { tradeIdeas: rawIdeas, loading, error } = useSymbolContext();
      ...
      <div className="max-h-[72vh] overflow-y-auto border border-zinc-800 rounded">
        <table className="w-full text-xs table-auto">
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr>
                <td colSpan="8" className="text-center text-gray-500 py-6 text-xs whitespace-nowrap">
            ...
            {filtered.map((item, i) => (
              ...
              >
                <td className={`px-0.5 py-0.5 font-bold text-xs ${ASSET_CLASS_COLORS[item.assetClass] || ASSET_CLASS_COLORS.Default}`}>
                  {item.symbol}
                  {item.isNew && item.phase === 'Phase 2' && (
                    <span className="ml-2 px-2 py-0.5 bg-green-800 text-green-100 text-xs rounded-full animate-pulse">
                      🟢 NEW
                    </span>
                  )}
                </td>
